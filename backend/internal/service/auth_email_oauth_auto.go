...
func (s *AuthService) createEmailOAuthUser(ctx context.Context, email, username, providerType, invitationCode, affiliateCode string) (*User, error) {
	...
	if err := s.bindAffiliateProfileForSignup(ctx, user.ID, affiliateCode); err != nil {
		logger.LegacyPrintf("service.auth", "[Auth] Failed to bind custom referral inviter for user %d during email oauth auto registration: %v", user.ID, err)
	}
