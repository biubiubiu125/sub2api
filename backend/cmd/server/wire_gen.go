...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	customReferralRepository := repository.NewCustomReferralRepository(client, db)
	customReferralService := service.NewCustomReferralService(customReferralRepository, settingRepository, configConfig)
	authService := service.ProvideAuthService(client, userRepository, redeemCodeRepository, refreshTokenCache, configConfig, settingService, emailService, turnstileService, emailQueueService, promoService, subscriptionService, customReferralService)
	...
	authHandler := handler.ProvideAuthHandler(configConfig, authService, userService, settingService, promoService, redeemService, totpService, customReferralService, userAttributeService)
	userHandler := handler.ProvideUserHandler(userService, authService, emailService, emailCache)
	...
	paymentService := service.ProvidePaymentService(client, registry, defaultLoadBalancer, redeemService, subscriptionService, paymentConfigService, userRepository, groupRepository, affiliateService, customReferralService, notificationEmailService)
	...
	providerPriceHandler := admin.NewProviderPriceAdminHandler(settingService)
	...
	referralAssetService := service.NewReferralAssetService()
	referralHandler := admin.NewReferralHandler(customReferralService, referralAssetService)
	adminHandlers := handler.ProvideAdminHandlers(dashboardHandler, adminUserHandler, groupHandler, accountHandler, adminAnnouncementHandler, dataManagementHandler, backupHandler, oAuthHandler, openAIOAuthHandler, geminiOAuthHandler, antigravityOAuthHandler, proxyHandler, adminRedeemHandler, promoHandler, settingHandler, opsHandler, systemHandler, adminSubscriptionHandler, adminUsageHandler, userAttributeHandler, errorPassthroughHandler, tlsFingerprintProfileHandler, adminAPIKeyHandler, scheduledTestHandler, channelHandler, providerPriceHandler, channelMonitorHandler, channelMonitorRequestTemplateHandler, contentModerationHandler, paymentHandler, referralHandler)
	...
	handlerReferralHandler := handler.NewReferralHandler(customReferralService, referralAssetService)
	groupService := service.NewGroupService(groupRepository, apiKeyAuthCacheInvalidator)
	handlerProviderPriceHandler := handler.NewProviderPriceHandler(groupService, paymentConfigService, billingService, channelService, settingService, gatewayService, modelPricingResolver)
	idempotencyCoordinator := service.ProvideIdempotencyCoordinator(idempotencyRepository, configConfig)
	idempotencyCleanupService := service.ProvideIdempotencyCleanupService(idempotencyRepository, configConfig)
	handlers := handler.ProvideHandlers(authHandler, userHandler, apiKeyHandler, usageHandler, redeemHandler, subscriptionHandler, announcementHandler, channelMonitorUserHandler, adminHandlers, gatewayHandler, openAIGatewayHandler, handlerSettingHandler, totpHandler, handlerPaymentHandler, paymentWebhookHandler, availableChannelHandler, handlerReferralHandler, handlerProviderPriceHandler, idempotencyCoordinator, idempotencyCleanupService)
